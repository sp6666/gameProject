...
import { EventManagerCallFunc } from "../manager/EventManager";
...
export class NetNode {
  ...
  public init(
    ...
    networkTips: any = null,
    execFunc: ExecuterFunc = null
  ) {
    console.log(`NetNode init socket`);
    this._socket = socket;
    this._protocolHelper = protocol;
    this._networkTips = networkTips;
    this._callbackExecuter = execFunc
      ? execFunc
      : (callback: CallbackObject, buffer: NetData) => {
  ...
  protected onMessage(msg: any): void {
    ...
    let jsonObj = JSON.parse(
      utils.byteToString(msgArr.slice(8, msgArr.length))
    );
      ...
      for (let i in data) {
        if (i == str) {
          for (let tip in data[i]) {
            ...
            if (null != s) {
              this.debugMode &&
                cc.warn("[NetNode]MOD:" + i + " type:" + tip + " accept");
              ...
              this.debugMode &&
                cc.warn("[NetNode][NOT-EXISTS]MOD:" + i + " type:" + tip);
          }
          break;
        }
      }
  ...
  protected getNetListenersIndex(
    cmd: number,
    rspObject: CallbackObject
  ): number {
    let index = -1;
    for (let i = 0; i < this._listener[cmd].length; i++) {
      ...
      if (
        iterator.callback == rspObject.callback &&
