...
import { UIView, UIShowTypes, UILevel } from "./UIView";
...
export class UIManager {
  ...
  private preventTouch(zOrder: number) {
    ...
    node.on(
      ...
      function (event: cc.Event.EventCustom) {
   ...
   */
  private autoExecAnimation(
    uiView: UIView,
    aniName: string,
    aniOverCallback: () => void
  ) {
    // 暂时先省略动画播放的逻辑
    aniOverCallback();
  }
  ...
  private getOrCreateUI(
    uiId: number,
    processCallback: (finish: number, total: number, item: any) => void,
    completeCallback: (uiView: UIView) => void,
    uiArgs: any
  ): void {
    // 如果找到缓存对象，则直接返回
    ...
    if (null == uiPath) {
      cc.log(`getOrCreateUI ${uiId} failed, prefab conf not found!`);
    ...
    cc.resources.load(
      ...
      (err: Error, prefab: cc.Prefab) => {
        ...
        if (err) {
          cc.log(
            `getOrCreateUI loadRes ${uiId} failed, path: ${uiPath} error: ${err}`
          );
          completeCallback(null);
          return;
        }
        ...
        if (null == uiNode) {
          cc.log(`getOrCreateUI instantiate ${uiId} failed, path: ${uiPath}`);
          completeCallback(null);
          cc.resources.release(uiPath, cc.Prefab);
          return;
        }
        // 检查组件获取错误
        uiView = uiNode.getComponent(UIView);
        if (null == uiView) {
          cc.log(`getOrCreateUI getComponent ${uiId} failed, path: ${uiPath}`);
          uiNode.destroy();
          completeCallback(null);
          cc.resources.release(uiPath, cc.Prefab);
          return;
  ...
  private onUIOpen(uiId: number, uiView: UIView, uiInfo: UIInfo, uiArgs: any) {
    ...
    if (this.UIStack.length > 1) {
      for (let count = this.UIStack.length - 1; count >= 0; count--) {
        if (
          this.UIStack[count].uiView.uiLevel === UILevel.BottomUI &&
          this.UIStack[count].uiView != uiView
        ) {
  ...
  public open(
    uiId: number,
    uiArgs: any = null,
    progressCallback: (finish: number, total: number, item: any) => void = null
  ): void {
    var uiInfo: UIInfo = {
      uiId: uiId,
      uiArgs: uiArgs,
    ...
    this.getOrCreateUI(
      ...
      (uiView: UIView): void => {
        ...
        if (uiInfo.isClose || null == uiView) {
          cc.log(`getOrCreateUI ${uiId} failed!
