...
export default class AudioManager {
   ...
   * 通用方法
   */
  public PlayEffect(resName: string, loop = false): void {
    var url = resName;
    if (SysDef.isNeedRemote) {
      url = SysDef.getAudioEffectUrl(resName);
    }
    var self = this;
    cc.resources.load(url, function (error: Error, resources: any) {
  ...
  public PlayMusic(resName: string) {
    var url = resName;
    if (SysDef.isNeedRemote) {
  ...
  public PlayPlotMusic(resName: string) {
    var url = resName;
    if (SysDef.isNeedRemote) {
      url = SysDef.getAudioMusicUrl(resName);
    }
  ...
  public SetEffectVol(vol: number) {
    this.effectVol = vol;
