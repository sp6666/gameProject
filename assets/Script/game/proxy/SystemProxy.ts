...
export default class SystemProxy {
  ...
  init(zone, serverTime) {
    ...
    this._timezoneOffset =
      3600000 * (this._timezoneClient - this._timezoneServer);
  ...
  isSameWeek(t, e) {
    return (
      !(t - e >= 604800) &&
      (t - this._timeOfMonday) / 604800 == (e - this._timeOfMonday) / 604800
    );
  }
  ...
  second2hms(t, format) {
    if (t > 86400 && null == format) {
      ...
      return (
        Math.floor(t / 86400) +
        dataManager.GetTextById(415) +
        (o > 0 ? o + dataManager.GetTextById(416) : "")
      );
    }
