...
export default class RenderList extends cc.Component {
  ...
  public set data(data) {
    ...
    if (null != this._data) {
      ...
      if (this.node_tips) this.node_tips.active = false;
      if (0 === this._data.length) {
        if (this.node_tips) this.node_tips.active = true;
        this.node.width = 0;
        this.node.height = 0;
  ...
  public set selectData(data: any) {
    null != this._data && (this.selectIndex = this._data.indexOf(data));
  }
    ...
    if (renderData) {
      this._itemHeight =
        0 == this._itemHeight || null == this._itemHeight
          ? renderData.node.height
          : this._itemHeight;
      this._itemWidth =
        0 == this._itemWidth || null == this._itemWidth
          ? renderData.node.width
          : this._itemWidth;
      this._renders.push(renderData);
      this.node.addChild(o);
      if (this.isHorizonList) {
        renderData.node.x =
          (this._itemWidth + this.spaceX) * e + this.paddingLeft;
        renderData.node.y = -this.paddingTop;
      } else {
        renderData.node.x =
          (this._itemWidth + this.spaceX) * (e % this.repeatX) +
          this.paddingLeft;
        renderData.node.y =
          -(this._itemHeight + this.spaceY) * Math.floor(e / this.repeatX) -
          this.paddingTop;
        this.isReversY && (renderData.node.y = -renderData.node.y);
      }
  ...
  createBuffer() {
    ...
    if (this.isSynchronizedUpdating) {
      ...
      executeAsync.setCallBack(
        this,
        this.createRenders.bind(this),
        this.startOnetime
      );
  ...
  updateShow() {
    ...
    if (this.isHorizonList) {
      ...
      e = Math.floor((t - this.paddingLeft) / (this._itemWidth + this.spaceX));
    } else {
      t = this.scrollView ? this.scrollView.getScrollOffset().y : 0;
      e = Math.floor((t - this.paddingTop) / (this._itemHeight + this.spaceY));
    }
    e = Math.min(e, (this._data.length - this.bufferZone) / this.repeatX);
    if ((e = Math.max(e, 0)) != this.lastIndex) {
      var o = this._renders.length;
      this.lastIndex = e;
      e *= this.repeatX;
      for (var i = this.getLastIndexes(e), n = 0; n < o; n++) {
        var l = this._data.length > n + e ? this._data[n + e] : null;
        if (null != l) {
          ...
          if (-1 != r) {
            ...
            if (null != a) {
              this._itemHeight != a.node.height &&
                0 != this._itemHeight &&
                a.setWidthHeigth(this._itemWidth, this._itemHeight);
              if (this.isHorizonList) {
                a.node.x =
                  (this._itemWidth + this.spaceX) * (n + e) + this.paddingLeft;
                a.node.y = -this.paddingTop;
              } else {
                a.node.x =
                  (this._itemWidth + this.spaceX) * ((n + e) % this.repeatX) +
                  this.paddingLeft;
                a.node.y =
                  -(this._itemHeight + this.spaceY) *
                    Math.floor((n + e) / this.repeatX) -
                  this.paddingTop;
                this.isReversY && (a.node.y = -a.node.y);
              }
  ...
  renderNext() {
    ...
    if (null != this._data && 0 != this._data.length) {
      ...
      if (this.isHorizonList) {
        this.node.height =
          this._itemHeight + this.spaceY + this.paddingTop + this.paddingBottom;
        this.node.width =
        ...
        this.node.height =
          (e - 1) * (this._itemHeight + this.spaceY) +
          this._itemHeight +
          this.paddingTop +
          this.paddingBottom;
        this.node.width =
          (t - 1) * (this._itemWidth + this.spaceX) +
          this._itemWidth +
          this.paddingLeft +
          this.paddingRight;
        null != this.content &&
  ...
  setWidthHeight(t: number, e: number) {
    ...
    if (null != this._renders)
      for (var o = 0; o < this._renders.length; o++)
        this._renders[o].setWidthHeight(t, e);
  }
   ...
   * 具体为
   *
   * 1. 进入ScrollView可视区域是，回调对应 Content 子节点上挂载的 ScrollViewPlusItem 组件的 onEnterScrollViewEvents 数组事件
   * 2. 退出ScrollView可视区域是，回调对应 Content 子节点上挂载的 ScrollViewPlusItem 组件的 onExitScrollViewEvents 数组事件
   */
