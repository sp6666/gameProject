...
export default class RoleSpine extends RenderListItem {
  ...
  onDestroy() {
    ...
    if (this.self) {
      EventMgr.removeEventListener(
        ClotheProxy.clothesWear,
        this.initSpine,
        this
      );
    }
  ...
  generateSpinesByData() {
    ...
    for (var index in this.data) {
      if (this.data[index] > 0) this.count++;
    }
    for (var index in this.data) {
  ...
  CreateSpineBySpineId(id, base = false) {
    ...
    for (var i = 0; i < spineData.spine.length; i++) {
      ...
      var self = this;
      let index = i;
      cc.resources.load(url, sp.SkeletonData, (error, res) => {
        if (error != null) {
          return;
        }
        ...
        let bones = self.attachUtil.generateAttachedNodes(
          spineData.solt[index]
        );
        if (id == 2102) {
          _node.zIndex = bones[0].zIndex = utils.getBoneIndex("r_arm");
        } else {
          _node.zIndex = bones[0].zIndex = utils.getBoneIndex(
            spineData.solt[index]
          );
        }
