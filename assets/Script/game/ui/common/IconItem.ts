...
export default class IconItem extends RenderListItem {
  ...
  showData() {
    ...
    if (this.sprBg) {
      this.sprBg.url = SysDef.getItemBg(
        this._data.quality ? this._data.quality : 1
      );
    }
    if (this.sprIcon) {
    ...
    if (this.lbName)
      this.lbName.string = dataManager.GetTextById(this._data.name_id);
    ...
    if (this.lbDec)
      this.lbDec.string = dataManager.GetTextById(this._data.describe_id);
    if (this.nodeAdd) {
